...
install_magento() {
  if [ ! -f /var/www/html/magento2/app/etc/env.php ]; then
		composer create-project --repository=https://repo.magento.com/ magento/project-community-edition /var/www/html/magento2
		cd /var/www/html/magento2
		bin/magento setup:install --base-url=http://magento2.local/ --db-host=127.0.0.1 --db-name=magento --db-user=magento --db-password=password --admin-firstname=admin --admin-lastname=admin --admin-email=msmohotti@gmail.com --admin-user=admin --admin-password=admin123 --language=en_US --currency=AED --timezone=Asia/Dubai --use-rewrites=1
  fi
}
